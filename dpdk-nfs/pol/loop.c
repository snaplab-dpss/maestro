...
void loop_reset(struct Map** dyn_map, struct Vector** dyn_keys,
                struct DoubleChain** dyn_heap, struct Vector** dyn_vals,
...
void loop_iteration_border(struct Map** dyn_map, struct Vector** dyn_keys,
                           ...
                           uint32_t dev_count, unsigned int lcore_id,
                           vigor_time_t time) {
  loop_invariant_consume(dyn_map, dyn_keys, dyn_heap, dyn_vals, capacity,
                         dev_count, lcore_id, time);
  loop_reset(dyn_map, dyn_keys, dyn_heap, dyn_vals, capacity, dev_count,
             lcore_id, &time);
  loop_invariant_produce(dyn_map, dyn_keys, dyn_heap, dyn_vals, capacity,
                         dev_count, &lcore_id, &time);
}
