...
/*@
inductive FlowIdi = FlowIdc(uint16_t , uint16_t , uint32_t , uint32_t , uint8_t ); @*/
...
predicate FlowIdp(struct FlowId* ptr; FlowIdi v) = 
  ...
  v == FlowIdc(src_port_f, dst_port_f, src_ip_f, dst_ip_f, protocol_f); @*/

/*@
fixpoint unsigned _FlowId_hash(FlowIdi x) {
  switch(x) { case FlowIdc(src_port_f, dst_port_f, src_ip_f, dst_ip_f, protocol_f):
    return crc32_hash(crc32_hash(crc32_hash(crc32_hash(crc32_hash(0, src_port_f), dst_port_f), src_ip_f), dst_ip_f), protocol_f);
  }
} @*/
