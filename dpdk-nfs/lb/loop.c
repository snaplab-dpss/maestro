...
void loop_reset(struct Map** flow_to_flow_id, struct Vector** flow_heap,
...
void loop_invariant_consume(
  ...
  klee_trace_param_ptr(flow_id_to_backend_id, sizeof(struct Vector*),
                       "flow_id_to_backend_id");
  klee_trace_param_ptr(ip_to_backend_id, sizeof(struct Map*),
                       "ip_to_backend_id");
  ...
  klee_trace_param_ptr(active_backends, sizeof(struct DoubleChain*),
                       "active_backends");
...
void loop_iteration_border(
  ...
  loop_invariant_consume(flow_to_flow_id, flow_heap, flow_chain,
                         ...
                         flow_capacity, cht_height, lcore_id, time);
  loop_reset(flow_to_flow_id, flow_heap, flow_chain, flow_id_to_backend_id,
             ip_to_backend_id, backend_ips, backends, active_backends, cht,
             backend_capacity, flow_capacity, cht_height, lcore_id, &time);
  loop_invariant_produce(flow_to_flow_id, flow_heap, flow_chain,
                         flow_id_to_backend_id, ip_to_backend_id, backend_ips,
                         backends, active_backends, cht, backend_capacity,
                         flow_capacity, cht_height, &lcore_id, &time);
}
