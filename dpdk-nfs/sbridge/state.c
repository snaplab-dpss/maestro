...
bool dyn_val_condition(void* value, int index, void* state) {
  ...
  return (0 <= v->device) AND(v->device < 2);
}

bool stat_map_condition(void* value, int index, void* state) {
  struct StaticKey* v = value;
  return (0 <= index) AND(index < 2);
}

struct State* alloc_state(uint32_t capacity, uint32_t stat_capacity,
                          uint32_t dev_count) {
...
#ifdef KLEE_VERIFICATION
  map_set_layout(
      ret->dyn_map, rte_ether_addr_descrs,
      sizeof(rte_ether_addr_descrs) / sizeof(rte_ether_addr_descrs[0]),
      rte_ether_addr_nests,
      sizeof(rte_ether_addr_nests) / sizeof(rte_ether_addr_nests[0]),
      "rte_ether_addr");
  vector_set_layout(
      ret->dyn_keys, rte_ether_addr_descrs,
      sizeof(rte_ether_addr_descrs) / sizeof(rte_ether_addr_descrs[0]),
      rte_ether_addr_nests,
      sizeof(rte_ether_addr_nests) / sizeof(rte_ether_addr_nests[0]),
      "rte_ether_addr");
...
void nf_loop_iteration_border(unsigned lcore_id, vigor_time_t time) {
  loop_iteration_border(
      ...
      &allocated_nf_state->st_vec, &allocated_nf_state->dyn_heap,
