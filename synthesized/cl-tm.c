...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0xc, 0x2e, 0x63, 0xea, 0x7c, 0xe0, 0x73, 0x71, 
  0x87, 0xe3, 0xb0, 0x6e, 0xec, 0x19, 0xa0, 0xcf, 
  0x46, 0xbd, 0x57, 0x2e, 0x96, 0x82, 0xd3, 0xb3, 
  0xfe, 0xa6, 0x27, 0x42, 0x22, 0x5c, 0x71, 0x2f, 
  0x8a, 0xd5, 0x19, 0x7, 0xb5, 0x8c, 0x78, 0x3c, 
  0x70, 0x28, 0xaa, 0x5c, 0x41, 0x4b, 0x2c, 0x87, 
  0x8, 0x83, 0xb5, 0x9f
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0xd0, 0x1f, 0xf3, 0xf1, 0x4b, 0xde, 0x2f, 0xb8, 
  0x4, 0x35, 0x17, 0x82, 0xb0, 0x9b, 0x12, 0x28, 
  0xdf, 0xa9, 0xab, 0x8c, 0x34, 0x9d, 0xc0, 0xd3, 
  0xa0, 0x2, 0x1d, 0x38, 0xe1, 0x93, 0xd1, 0xb2, 
  0xb2, 0xc5, 0xa3, 0xfd, 0xa3, 0xd2, 0xb5, 0xa7, 
  0x7, 0xcc, 0x29, 0xb7, 0x67, 0x3b, 0xdf, 0x46, 
  0xe4, 0x8a, 0xd3, 0x18
};
...
bool nf_init() {
  ...
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_allocate(13u, 65536u, flow_allocate, &vector);
    ...
    if (vector_alloc_success__4) {
      ...
      if (is_dchain_allocated__7) {
        int sketch_allocation_succeeded__10 = sketch_tm_allocate(client_hash, 1024u, 64u, &sketch);
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      int number_of_freed_flows__36 = expire_items_single_map_tm(dchain, vector, map, now - 100000000000ul);
      sketch_tm_expire(sketch, now - 100000000000ul);
      ...
      // 123
        ...
        sketch_tm_compute_hashes(sketch, &sketch_key);
        ...
        if (0u == map_has_this_key__48) {
          ...
          int out_of_space__52 = !dchain_tm_allocate_new_index(dchain, &new_index__52, now);
          ...
          if (false == ((out_of_space__52) & (0u == number_of_freed_flows__36))) {
            ...
            int overflow__58 = sketch_tm_fetch(sketch);

            // 120
            if (0u == overflow__58) {
              int success__61 = sketch_tm_touch_buckets(sketch, now);
              return 1;
            }
