...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0x3f, 0x33, 0x92, 0x72, 0xa0, 0x65, 0xec, 0xf6, 
  0xd5, 0xe1, 0xd1, 0xe6, 0xc9, 0x87, 0x2e, 0x93, 
  0x35, 0x40, 0xd8, 0xf7, 0xd9, 0x7c, 0x28, 0xa4, 
  0x90, 0xeb, 0x4e, 0x97, 0xb7, 0x93, 0x67, 0xf6, 
  0xc6, 0xf9, 0x69, 0x66, 0x5e, 0x55, 0x5c, 0x33, 
  0x36, 0x2e, 0x1a, 0xff, 0xb5, 0x48, 0x92, 0xea, 
  0x89, 0x6a, 0xe1, 0x62
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0xfb, 0x62, 0x8, 0x7d, 0xb9, 0x43, 0x7, 0xaa, 
  0x83, 0xd6, 0x86, 0x19, 0x19, 0x4d, 0xd6, 0x95, 
  0x70, 0x27, 0xe3, 0x31, 0x9f, 0xb3, 0x52, 0x74, 
  0x46, 0xe4, 0xd4, 0xcd, 0x19, 0xfd, 0x1b, 0x14, 
  0x5f, 0x24, 0x91, 0x18, 0x67, 0x98, 0xc2, 0xea, 
  0x6e, 0x48, 0x4, 0x88, 0x96, 0xda, 0x1d, 0x6, 
  0x1, 0x0, 0x37, 0xa0
};
  ...
  if (map_allocation_succeeded__1) {
    ...
    if (vector_alloc_success__4) {
      ...
      if (vector_alloc_success__7) {
        int map_allocation_succeeded__10 = map_locks_allocate(StaticKey_eq, StaticKey_hash, 8192u, &map_1);

        // 82
        // 83
        // 84
        if (map_allocation_succeeded__10) {
          int vector_alloc_success__13 = vector_locks_allocate(8u, 8192u, StaticKey_allocate, &vector_2);

          // 82
          // 83
          if (vector_alloc_success__13) {
            int is_dchain_allocated__16 = dchain_locks_allocate(65536u, &dchain);
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  ...
  int map_value_out;
  int map_has_this_key__34 = map_locks_get(map_1, map_key, &map_value_out);
