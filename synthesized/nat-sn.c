...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  ...
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 
  0x35, 0x26, 0x3b, 0x2a, 0x79, 0x8b, 0x7d, 0x2a, 
  0xb9, 0xbd, 0x8a, 0x1f, 0xae, 0x8c, 0xfb, 0x4e, 
  0x9b, 0xb3, 0x79, 0xc7, 0xf6, 0xd3, 0x32, 0xf7, 
  0x66, 0x1d, 0x19, 0xf8, 0xc1, 0xeb, 0x69, 0xf6, 
  0x12, 0xa5, 0x21, 0x8b
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 
  0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
  0x46, 0xed, 0x50, 0xf0, 0xbe, 0x84, 0x47, 0x65, 
  0xf, 0x14, 0x62, 0x3b, 0x4e, 0x21, 0xe4, 0x83, 
  0x75, 0x93, 0x7f, 0xbd, 0x6c, 0x61, 0x30, 0xcb, 
  0x88, 0xd4, 0x78, 0xac, 0x55, 0x34, 0x24, 0x9b, 
  0x21, 0x74, 0x8b, 0xdf
};
...
bool nf_init() {
  ...
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_allocate(16u, spread_data_among_cores(65536u), FlowId_allocate, &(*vector_ptr));
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      // 127
      if (0u != device) {
        int32_t dchain_is_index_allocated__36 = dchain_is_index_allocated((*dchain_ptr), tcpudp_header_1->dst_port);

        // 125
        // 126
        if (dchain_is_index_allocated__36) {
          uint8_t* vector_value_out = 0u;
          vector_borrow((*vector_ptr), tcpudp_header_1->dst_port, (void**)(&vector_value_out));
          vector_return((*vector_ptr), tcpudp_header_1->dst_port, vector_value_out);
      ...
      // 130
        ...
        if (0u == map_has_this_key__68) {
          ...
          if (false == ((out_of_space__71) & (0u == number_of_freed_flows__24))) {
            ...
            vector_return((*vector_ptr), new_index__71, vector_value_out);
