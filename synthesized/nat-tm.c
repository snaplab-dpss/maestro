...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0xb4, 0xd6, 0xe6, 0x9d, 0xc0, 0x26, 0x2e, 0xb0, 
  0xce, 0x10, 0xc3, 0xec, 0x1a, 0x42, 0x19, 0xe6, 
  0xfe, 0xea, 0x2, 0x3b, 0xb5, 0x34, 0x3d, 0xe, 
  0xd8, 0x42, 0xd, 0xf6, 0xa9, 0x58, 0xbf, 0x5d, 
  0x2e, 0xa5, 0xfa, 0xef, 0xcc, 0x28, 0x9f, 0x9a, 
  0x39, 0x62, 0x86, 0x53, 0xa4, 0x9f, 0x39, 0xa2, 
  0x89, 0x3c, 0xdd, 0x3e
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0xf3, 0x28, 0xa9, 0x7b, 0x64, 0x6, 0x25, 0x9f, 
  0x83, 0xa5, 0xc7, 0xf6, 0x34, 0xee, 0xbe, 0xd4, 
  0x34, 0x5b, 0x90, 0x51, 0x65, 0xe0, 0x6d, 0xab, 
  0x75, 0x5f, 0x9a, 0x71, 0x5a, 0x5f, 0xb9, 0x4d, 
  0x87, 0x62, 0xc8, 0xeb, 0x68, 0xed, 0x8a, 0xeb, 
  0x92, 0x51, 0xe2, 0xc6, 0x40, 0xa0, 0x9b, 0x74, 
  0xfc, 0x2b, 0xc6, 0x61
};
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  int number_of_freed_flows__24 = expire_items_single_map_tm(dchain, vector, map, now - 100000000000ul);
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      // 127
      if (0u != device) {
        int32_t dchain_is_index_allocated__36 = dchain_tm_is_index_allocated(dchain, tcpudp_header_1->dst_port);
