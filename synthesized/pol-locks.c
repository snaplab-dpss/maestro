...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0x2c, 0x72, 0xcf, 0xe4, 0xf3, 0x8c, 0x29, 0xd1, 
  0xfe, 0x2b, 0x4c, 0x75, 0xa3, 0xbc, 0x3d, 0x2b, 
  0xe8, 0xd7, 0x48, 0x92, 0xfe, 0xf4, 0x49, 0x50, 
  0xdf, 0x63, 0x3a, 0xd1, 0x1c, 0xb5, 0xa, 0x48, 
  0x27, 0xd9, 0x2d, 0x1a, 0x66, 0x56, 0xec, 0x64, 
  0x81, 0x38, 0xda, 0x24, 0xf4, 0x17, 0x50, 0xdc, 
  0xef, 0x98, 0x6e, 0xed
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0x86, 0xd7, 0x31, 0xd1, 0x79, 0xaa, 0xda, 0xaf, 
  0x18, 0xd0, 0x5f, 0x3c, 0xef, 0xd5, 0xba, 0xff, 
  0xf, 0x3e, 0x59, 0x6d, 0xb4, 0x4, 0x62, 0xd1, 
  0xcb, 0x18, 0x79, 0xd2, 0x2d, 0xb9, 0x97, 0xb3, 
  0x91, 0xc8, 0x85, 0xa, 0x72, 0x5f, 0xb9, 0x8a, 
  0x2f, 0x18, 0xc7, 0x1f, 0xee, 0x81, 0x1e, 0xfd, 
  0xbf, 0x77, 0x6a, 0x73
};
  ...
  if (map_allocation_succeeded__1) {
    ...
    if (vector_alloc_success__4) {
      ...
      if (is_dchain_allocated__7) {
        int vector_alloc_success__10 = vector_locks_allocate(16u, 65536u, DynamicValue_allocate, &vector_1);
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    int number_of_freed_flows__32 = expire_items_single_map_locks(dchain, vector, map, now - 10000000000ul);
    ...
    // 129
      ...
      int map_value_out;
      int map_has_this_key__42 = map_locks_get(map, &map_key, &map_value_out);

      // 124
      // 125
      if (0u == map_has_this_key__42) {
        uint32_t new_index__45;
        int out_of_space__45 = !dchain_locks_allocate_new_index(dchain, &new_index__45, now);
        ...
        if (false == ((out_of_space__45) & (0u == number_of_freed_flows__32))) {
          ...
          vector_locks_borrow(vector, new_index__45, (void**)(&vector_value_out));
          ...
          vector_locks_borrow(vector_1, new_index__45, (void**)(&vector_value_out_1));
          ...
          map_locks_put(map, vector_value_out, new_index__45);

          if (write_attempt_ptr[0] && (!write_state_ptr[0])) {
            return 1;
          }

          vector_locks_return(vector, new_index__45, vector_value_out);
          vector_locks_return(vector_1, new_index__45, vector_value_out_1);
          return 1;
        }
