...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0xe7, 0x1d, 0x44, 0xd8, 0xe4, 0x38, 0x9d, 0xed, 
  0x9b, 0x44, 0x94, 0xbf, 0x15, 0x8a, 0x9, 0x7d, 
  0x25, 0xd6, 0x2, 0xb9, 0x50, 0x52, 0xcb, 0xc7, 
  0x59, 0x75, 0x41, 0xa3, 0x56, 0x76, 0xa, 0x3d, 
  0x94, 0x4e, 0x15, 0x78, 0x86, 0xb2, 0x66, 0x22, 
  0xf6, 0xfa, 0xe1, 0xc, 0x84, 0xea, 0x89, 0xa9, 
  0xc0, 0x8b, 0x63, 0x10
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0x1, 0x48, 0xd9, 0xf9, 0xbb, 0x37, 0x1d, 0x6f, 
  0x72, 0x7a, 0xc4, 0xa1, 0x4f, 0x1f, 0x77, 0x10, 
  0x7a, 0xb2, 0x58, 0x38, 0x4b, 0x3d, 0xb2, 0x9b, 
  0x2f, 0x3b, 0x75, 0x25, 0xe3, 0x26, 0xb3, 0xe4, 
  0x6e, 0x8d, 0xde, 0x29, 0xc4, 0xfb, 0x99, 0x36, 
  0x75, 0x5d, 0xd8, 0xc4, 0x7c, 0x4f, 0xd5, 0xf6, 
  0x2, 0x2d, 0x2f, 0x4d
};
...
bool nf_init() {
  ...
  if (map_allocation_succeeded__1) {
    ...
    if (vector_alloc_success__4) {
      int vector_alloc_success__7 = vector_locks_allocate(4u, 65536u, null_init, &vector_1);
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      if (0u != device) {
        ...
        int map_value_out;
        int map_has_this_key__39 = map_locks_get(map, &map_key, &map_value_out);
      ...
      // 124
        ...
        map_key[12u] = ipv4_header_1->next_proto_id;
        int map_value_out;
        int map_has_this_key__61 = map_locks_get(map, &map_key, &map_value_out);

        // 122
        // 123
        if (0u == map_has_this_key__61) {
          uint32_t new_index__64;
          int out_of_space__64 = !dchain_locks_allocate_new_index(dchain, &new_index__64, now);
          ...
          if (false == ((out_of_space__64) & (0u == number_of_freed_flows__27))) {
            ...
            uint8_t* vector_value_out = 0u;
            vector_locks_borrow(vector, new_index__64, (void**)(&vector_value_out));
            ...
            map_locks_put(map, vector_value_out, new_index__64);

            if (write_attempt_ptr[0] && (!write_state_ptr[0])) {
              return 1;
            }

            vector_locks_return(vector, new_index__64, vector_value_out);
            ...
            vector_locks_borrow(vector_1, new_index__64, (void**)(&vector_value_out_1));
            vector_value_out_1[0u] = device & 0xff;
            vector_value_out_1[1u] = (device >> 8) & 0xff;
            vector_value_out_1[2u] = 0u;
            vector_value_out_1[3u] = 0u;
            vector_locks_return(vector_1, new_index__64, vector_value_out_1);
            ether_header_1->d_addr.addr_bytes[0ul] = 1u;
            ether_header_1->d_addr.addr_bytes[1ul] = 35u;
            ether_header_1->d_addr.addr_bytes[2ul] = 69u;
            ether_header_1->d_addr.addr_bytes[3ul] = 103u;
