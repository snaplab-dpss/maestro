...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0x88, 0x9e, 0x99, 0x86, 0x81, 0xaf, 0xec, 0x9, 
  0x9e, 0x3e, 0xbc, 0x74, 0xbc, 0x4b, 0xa1, 0xe0, 
  0x19, 0x42, 0x2f, 0xdc, 0xc8, 0x6a, 0x9a, 0x17, 
  0x3e, 0x1d, 0x84, 0x4e, 0xd9, 0xa4, 0x9c, 0x61, 
  0x43, 0x35, 0xe7, 0xc4, 0xe4, 0xd4, 0xce, 0x82, 
  0x12, 0x8a, 0xf6, 0xce, 0xd6, 0x97, 0xaf, 0xef, 
  0xd9, 0xde, 0xcc, 0xa1
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0xa9, 0x2, 0xa9, 0x64, 0x22, 0xe9, 0x88, 0x62, 
  0xe2, 0x63, 0xed, 0x94, 0xb6, 0xda, 0x77, 0xfe, 
  0xf9, 0x49, 0xa8, 0x55, 0xec, 0x23, 0xac, 0x52, 
  0x61, 0x2a, 0x7f, 0x76, 0x4e, 0x72, 0x47, 0xf7, 
  0x74, 0xf0, 0x5b, 0x96, 0xd9, 0xe4, 0xf9, 0xbb, 
  0x47, 0xe6, 0x50, 0xfd, 0xc0, 0xc7, 0xfc, 0xb9, 
  0x10, 0xa4, 0xe, 0xfd
};
...
bool nf_init() {
  ...
  int map_allocation_succeeded__1 = map_locks_allocate(FlowId_eq, FlowId_hash, 65536u, &map);

  // 120
  // 121
  // 122
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_locks_allocate(15u, 65536u, FlowId_allocate, &vector);
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  bool* write_attempt_ptr = &RTE_PER_LCORE(write_attempt);
  bool* write_state_ptr = &RTE_PER_LCORE(write_state);
  int number_of_freed_flows__24 = expire_items_single_map_locks(dchain, vector, map, now - 100000000000ul);
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      // 127
      if (0u != device) {
        int32_t dchain_is_index_allocated__36 = dchain_locks_is_index_allocated(dchain, tcpudp_header_1->dst_port);

        // 125
        // 126
        if (dchain_is_index_allocated__36) {
          uint8_t* vector_value_out = 0u;
          vector_locks_borrow(vector, tcpudp_header_1->dst_port, (void**)(&vector_value_out));
          vector_locks_return(vector, tcpudp_header_1->dst_port, vector_value_out);

          // 125
          if (((vector_value_out[8ul] == ipv4_header_1->src_addr) & (vector_value_out[2ul] == tcpudp_header_1->src_port)) & (vector_value_out[14ul] == ipv4_header_1->next_proto_id)) {
          ...
          // 126
            ...
            // dropping
            return device;
          } // !(((vector_value_out[8ul] == ipv4_header_1->src_addr) & (vector_value_out[2ul] == tcpudp_header_1->src_port)) & (vector_value_out[14ul] == ipv4_header_1->next_proto_id))
      ...
      // 130
        ...
        if (0u == map_has_this_key__68) {
          ...
          if (false == ((out_of_space__71) & (0u == number_of_freed_flows__24))) {
            ...
            uint8_t* vector_value_out = 0u;
            vector_locks_borrow(vector, new_index__71, (void**)(&vector_value_out));
            ...
            vector_locks_return(vector, new_index__71, vector_value_out);
