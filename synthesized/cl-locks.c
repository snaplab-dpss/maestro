...
uint8_t hash_key_0[RSS_HASH_KEY_LENGTH] = {
  0xc8, 0x3e, 0xce, 0xd7, 0x2c, 0x91, 0xf7, 0x3, 
  0xe4, 0x14, 0x1b, 0x7, 0xea, 0x72, 0x12, 0x6a, 
  0xd2, 0x4b, 0x37, 0x34, 0x88, 0x76, 0x9a, 0xc5, 
  0x16, 0xa0, 0xaa, 0xd7, 0xb1, 0xd6, 0x3a, 0x79, 
  0x15, 0x8, 0x50, 0x41, 0x9a, 0x47, 0x45, 0x7e, 
  0x5b, 0x60, 0x85, 0x45, 0xd2, 0x97, 0xaf, 0xa5, 
  0xe2, 0xe6, 0xd9, 0x6b
};
uint8_t hash_key_1[RSS_HASH_KEY_LENGTH] = {
  0x5c, 0x52, 0x10, 0xf4, 0xc7, 0xef, 0xa5, 0x18, 
  0xfa, 0x48, 0x84, 0x75, 0x6f, 0x47, 0x7b, 0x6a, 
  0xf2, 0xdc, 0xbc, 0x15, 0xa5, 0xb5, 0xb4, 0x21, 
  0x5a, 0x3d, 0xab, 0x2a, 0xff, 0xa4, 0x3c, 0x5c, 
  0xf6, 0x4d, 0x50, 0xbd, 0x3c, 0xf5, 0xd5, 0x36, 
  0x3d, 0x59, 0xac, 0xad, 0xa1, 0x27, 0x17, 0x93, 
  0x3, 0xd3, 0xa8, 0xa9
};
...
bool nf_init() {
  ...
  if (map_allocation_succeeded__1) {
    int vector_alloc_success__4 = vector_locks_allocate(16u, 65536u, flow_allocate, &vector);
    ...
    if (vector_alloc_success__4) {
      ...
      if (is_dchain_allocated__7) {
        int sketch_allocation_succeeded__10 = sketch_locks_allocate(client_hash, 1024u, 64u, &sketch);
...
int nf_process(uint16_t device, uint8_t* packet, uint16_t packet_length, int64_t now) {
  ...
  if ((8u == ether_header_1->ether_type) & (20ul <= (4294967282u + packet_length))) {
    ...
    if (((6u == ipv4_header_1->next_proto_id) | (17u == ipv4_header_1->next_proto_id)) & ((4294967262u + packet_length) >= 4ul)) {
      ...
      int number_of_freed_flows__36 = expire_items_single_map_locks(dchain, vector, map, now - 100000000000ul);

      if (write_attempt_ptr[0] && (!write_state_ptr[0])) {
        return 1;
      }

      sketch_locks_expire(sketch, now - 100000000000ul);
      ...
      // 123
        ...
        int map_value_out;
        int map_has_this_key__48 = map_locks_get(map, &map_key, &map_value_out);
        ...
        sketch_locks_compute_hashes(sketch, &sketch_key);
        ...
        if (0u == map_has_this_key__48) {
          ...
          int out_of_space__52 = !dchain_locks_allocate_new_index(dchain, &new_index__52, now);
          ...
          if (false == ((out_of_space__52) & (0u == number_of_freed_flows__36))) {
            ...
            vector_locks_borrow(vector, new_index__52, (void**)(&vector_value_out));
            ...
            map_locks_put(map, vector_value_out, new_index__52);

            if (write_attempt_ptr[0] && (!write_state_ptr[0])) {
              return 1;
            }

            vector_locks_return(vector, new_index__52, vector_value_out);
            int overflow__58 = sketch_locks_fetch(sketch);

            // 120
            if (0u == overflow__58) {
              int success__61 = sketch_locks_touch_buckets(sketch, now);
        ...
        // 123
          ...
          sketch_locks_refresh(sketch, now);
          return 1;
