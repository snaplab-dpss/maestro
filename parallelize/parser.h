...
public:
  enum LoadedContentType {
    UNPARSED, ACCESS, ARGUMENT, PACKET_DEPENDENCIES, CHUNK, METADATA, EXPRESSION
  };

  union LoadedContent {
    struct unparsed_t { LoadedContentType type; std::string value; } unparsed;
    struct access_t { LoadedContentType type; LibvigAccess value; } access;
    struct argument_t { LoadedContentType type; LibvigAccessArgument value; } argument;
    struct expression_t { LoadedContentType type; std::string value; } expression;
    struct chunk_t { LoadedContentType type; std::shared_ptr<const Dependency> value; } chunk;
    struct dependencies_t { LoadedContentType type; std::vector< std::shared_ptr<const Dependency> > value; } dependencies;
    struct metadata_t { LoadedContentType type; LibvigAccessMetadata value; } metadata;

    LoadedContent(const std::string& _unparsed)
      : unparsed{UNPARSED, std::move(_unparsed)} {}

    LoadedContent(const LibvigAccess& _access)
    ...
    LoadedContent(const std::vector<std::string>& fragments) {
      ...
      for (auto frag : fragments) {
        frag.erase(frag.begin(), std::find_if(frag.begin(), frag.end(), [](int ch) {
    ...
    LoadedContent(const std::vector< std::shared_ptr<const Dependency> >& _dependencies)
  ...
  public:
    Content(const std::string& _unparsed)
      : content(std::move(_unparsed)) {}

    Content(const LibvigAccess& _access)
      : content(_access) {}

    Content(const LibvigAccessArgument& _arg)
      : content(_arg) {}

    Content(const std::vector<std::string>& fragments)
      : content(std::move(fragments)) {}

    Content(const std::vector< std::shared_ptr<const Dependency> >& _dependencies)
      : content(_dependencies) {}

    Content(std::shared_ptr<const Dependency> _chunk)
      : content(_chunk) {}

    Content(const LibvigAccessMetadata& _metadata)
      : content(_metadata) {}
    ...
    const LoadedContentType& get_loaded_type() const { return content.unparsed.type; }
  };
...
private:
  ...
  bool consume_token(const std::string &token, std::istringstream& iss, bool optional=false);
...
public:
  Parser(const std::string& filepath) : line_counter(0) {
    parse(filepath);
  }
