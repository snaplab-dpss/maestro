...
bool libvig_access_eq(libvig_access_t l1, libvig_access_t l2) {
    if (l1.device  != l2.device)  return false;
    if (l1.obj     != l2.obj)     return false;
    if (l1.layer   != l2.layer)   return false;
    if (l1.proto   != l2.proto)   return false;
    if (l1.deps.sz != l2.deps.sz) return false;
...
void libvig_accesses_append_unique(
    ...
    accesses->accesses = (libvig_access_t*) realloc(
        accesses->accesses,
        sizeof(libvig_access_t) * (accesses->sz)
    );
...
bool dep_eq(dep_t d1, dep_t d2) {
    return (d1.pf_is_set == d2.pf_is_set)
        && (!d1.pf_is_set || (d1.pf == d2.pf));
}
...
bool dep_in_array(deps_t deps, dep_t dep) {
    for (unsigned i = 0; i < deps.sz; i++)
        if (
            (dep.pf_is_set == deps.deps[i].pf_is_set)
...
void deps_append_unique(deps_t *deps, dep_t dep) {
    if (dep_in_array(*deps, dep)) return;
    
    deps->sz++;
    deps->deps = (dep_t*) realloc(
        deps->deps,
        sizeof(dep_t) * deps->sz
    );
