...
private:
    ...
    std::vector< std::pair<PacketFieldExpression, std::unique_ptr<const PacketDependency> > > packet_fields;
...
public:
  ...
  Constraint(const Constraint& constraint)
    : ctx(constraint.get_context()),
      first(constraint.get_first_access()),
      ...
      for (const auto& packet_field : constraint.get_packet_fields()) {
          packet_fields.emplace_back(packet_field.first, packet_field.second->clone());
      }
  }
  ...
  const std::vector< std::pair<PacketFieldExpression, std::unique_ptr<const PacketDependency> > >& get_packet_fields() const { return packet_fields; }
