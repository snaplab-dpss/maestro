...
private:
    ...
    std::vector<R3S_pf_t> unique_packet_fields_dependencies;
    std::vector<R3S_cnstrs_func> solver_constraints_generators;
...
private:
    ...
    void merge_unique_packet_field_dependencies(const std::vector<R3S_pf_t>& packet_fields);
...
public:
    ...
    const R3S_cfg_t& get_cfg() const { return cfg; }
    const std::vector<Constraint>& get_constraints() const { return constraints; }

    static Z3_ast ast_replace(Z3_context ctx, Z3_ast root, Z3_ast target, Z3_ast dst);
    static Z3_ast make_solver_constraints(R3S_cfg_t cfg, R3S_packet_ast_t p1, R3S_packet_ast_t p2);
