...
void loop_reset(struct Map **srcs, struct Vector **srcs_keys,
                ...
                struct Vector **ports_key, struct DoubleChain **ports_indexer,
                uint32_t capacity, uint32_t max_ports, uint32_t dev_count,
  ...
  dchain_reset(*ports_indexer, capacity * max_ports);
...
void loop_invariant_consume(struct Map **srcs, struct Vector **srcs_keys,
  ...
  klee_trace_param_ptr(ports_indexer, sizeof(struct DoubleChain *),
                       "ports_indexer");
...
void loop_iteration_border(struct Map **srcs, struct Vector **srcs_keys,
  ...
  loop_invariant_consume(srcs, srcs_keys, touched_ports_counter, allocator,
                         ports, ports_key, ports_indexer, capacity, max_ports,
                         dev_count, lcore_id, time);
  loop_reset(srcs, srcs_keys, touched_ports_counter, allocator, ports,
             ports_key, ports_indexer, capacity, max_ports, dev_count, lcore_id,
             &time);
