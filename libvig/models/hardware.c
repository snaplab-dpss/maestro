...
typedef uint32_t (*stub_register_read)(struct stub_device *dev,
                                       uint32_t offset);
typedef uint32_t (*stub_register_write)(struct stub_device *dev,
...
// We do not model RC registers specially because we don't allow writes to any
...
static void stub_device_start(struct stub_device *dev) {
  ...
  // Get device index
  int device_index = 0;
  while (dev != &DEVICES[device_index]) {
    device_index++;
  }
  ...
  // 32-63: RSS Hash or FCOE_PARAM or Flow Director Filters ID or Fragment
  // Checksum (0 - not supported)
  uint64_t wb0 =
...
static uint32_t stub_register_rw1c_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
  for (int n = 0; n <= 31; n++) {
...
static uint32_t stub_register_i2cctl_write(struct stub_device *dev,
                                           uint32_t offset,
                                           uint32_t new_value) {
  // I2C citations here are from
  // https://www.nxp.com/docs/en/user-guide/UM10204.pdf SFP citations here are
  // from https://ta.snia.org/higherlogic/ws/public/download/268/SFF-8431.PDF
  // and https://ta.snia.org/higherlogic/ws/public/download/294/SFF-8472.PDF

  // Table 10, "Characteristics of the SDA and SCL bus lines for Standard [...]
  ...
  // NOTE: We model the out bits as writable because the driver has to write to
  // them;
  //       otherwise, it would need to read I2CCTL every time before writing,
  //       which would be inefficient; so instead it just writes whatever to
  //       them and the value is ignored.
  ...
  if (dev->i2c_state == I2C_STARTED) {
    ...
    //  This address is seven bits long followed by an eight bit which is a data
    //  direction bit (R/W) - a 'zero' indicates a transmission (WRITE), a 'one'
    ...
    if (dev->i2c_counter % 9 < 8) {
      ...
      // "The Acknowledge signal is defined as follows: the transmitter releases
      // the SDA line
      //  during the acknowledge clock pulse so the receiver can pull the SDA
      ...
      dev->i2c_state =
          (dev->i2c_address & 1) == 0 ? SFP_ADDRESSING : SFP_READING;

      // Reset SFP address if we're about to address (otherwise, don't! reads
      // use the last one)
      ...
      // a START condition" "The first seven bits of the first byte are the
      // combination 1111 0XX" "Although there are eight possible combinations
      // of the reserved address bits 1111 xxx,
      //  only the four combinations 1111 0XX are used for 10-bit addressing.
      //  The remaining four combinations 1111 1XX are reserved for future
      ...
      // except that by "A0h" they mean "the 7-bit address 50h padded with a 0
      // at the end to make it 8 bit" we only suport A0 (for now?)
  ...
  if (dev->i2c_state == SFP_ADDRESSING) {
    if (dev->i2c_counter % 9 < 8) {
      ...
      // Put in write mode, which may be canceled by the master if it wants to
      // read by restarting
  ...
  if (dev->i2c_state == SFP_READING) {
    ...
    //  The check code shall be the low order 8 bits of the sum of the contents
    //  of all the bytes from byte 0 to byte 62, inclusive."
    uint32_t cc_base = 0;
    for (int n = 0; n <= 62; n++) {
...
static uint32_t stub_register_ctrl_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
  // Bit 2 is cleared once no master requests are pending, which we don't
  // emulate anyway
  SET_BIT(new_value, 2, 0);
...
static uint32_t stub_register_eerd_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
  uint32_t current_value = DEV_REG(dev, offset);
  ...
  // Same with the data
  for (int n = 16; n <= 31; n++) {
    klee_assert((GET_BIT(current_value, n) != 0) |
                (GET_BIT(new_value, n) != 1));
  }
  ...
  // and then for magic... this specific thing is a version, apparently? if it's
  // not >5, ixgbe assumes the eeprom is invalid
  eeprom_map[0x3F + 1 + 0xA + 1 + 0x7] = 0x6;

  // Checksum word - sum of all words from 0x00 to 0x3F (including words pointed
  // to if any, except the FW block) must be 0xBABA
  uint16_t checksum = 0;
...
static uint32_t stub_register_msca_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
  // page 135
  // MDIO Direct Access
  ...
  // Bit 30 says there is a command; start by clearing it, since we execute
  ...
  if (((new_value >> 28) & 0b11) == 0b00) {
    ...
    if (opcode == 0b00) {  // address operation
      ...
      // only support the reset register - we just pretend that it's been reset
      // all the time
...
static uint32_t stub_register_dbal_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
...
static uint32_t stub_register_tdh_write(struct stub_device *dev,
                                        uint32_t offset, uint32_t new_value) {
...
static uint32_t stub_register_needsrxen0_write(struct stub_device *dev,
                                               uint32_t offset,
                                               uint32_t new_value) {
...
static uint32_t stub_register_mtqc_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
...
static uint32_t stub_register_swsm_read(struct stub_device *dev,
                                        uint32_t offset) {
  uint32_t current_value = DEV_REG(dev, offset);
...
static uint32_t stub_register_swsm_write(struct stub_device *dev,
                                         uint32_t offset, uint32_t new_value) {
  uint32_t current_value = DEV_REG(dev, offset);
...
static uint32_t stub_register_swfwsync_write(struct stub_device *dev,
                                             uint32_t offset,
                                             uint32_t new_value) {
  // Cannot write to this register unless the software semaphore bit of SWSM is
  // taken
  klee_assert(GET_BIT(DEV_REG(dev, 0x10140), 1) == 1);

  uint32_t current_value = DEV_REG(dev, offset);

  // Cannot write 1 to a bit in this register if the firmware set the
  // corresponding bit
  for (int n = 0; n < 5; n++) {
...
#define REG(addr, val, mask)                           \
  ...
  // NOTE: "CTRL is also mapped to address 0x00004 to maintain compatibility
  ...
  // 18: IO Active (0 - not active; note: "reflects the value of the VF Enable
  // (VFE) bit in the IOV Control/Status register") 19: Status (0 - not issuing
  // any master requests) 20-31: Reserved (0x00)
  ...
  // 28: Unrecoverable ECC Error (0 - no) TODO try this, software must reset if
  // this is set 29: Reserved (0) 30: TCP Timer Expired (0 - no) 31: Other
  // interrupt - from EICR (0 - no)
  ...
  // 0-30: Interrupt Enable, each bit enables its corresponding interrupt in
  // EICR (0 - not enabled) 31: Reserved (0)
  ...
  // 0-31: Writing a 1b to any bit clears the corresponding bit in the EIMS[n]
  // register
  ...
  // page 621
  // Rx DCA Control Register — DCA_RXCTRL[n] (0x0100C + 0x40*n, n=0...63 and
  ...
  for (int n = 0; n <= 127; n++) {
    ...
    // 10: Reserved (0)
    // 11: RX Descriptor Write Back Relax Order Enabled (0 - read-only; "this
    // bit must be 0 to enable correct functionality") 12: Reserved (0) 13: RX
    // Data Write Relax Order Enable (1 - enabled) 14: Reserved (0) 15: RX Split
    // Header Relax Order Enable (1 - enabled) 16-23: Reserved (0) 24-31: CPU ID
    // (0 - not set)
  ...
  // 0-4: Receive Buffer Size for Packet Buffer (0x2 - default; "This field
  // should not be set to 0x0. This field should be greater or equal to 0x2 in
  // queues where RSC is enabled") 5-7: Reserved (0) 8-13: Receive Buffer Size
  ...
  // page 604
  // Transmit Descriptor Base Address Low — TDBAL[n] (0x06000+0x40*n, n=0...127;
  // RW)
  ...
  // page 605
  // Transmit Descriptor Base Address High — TDBAH[n] (0x06004+0x40*n,
  // n=0...127; RW)
  ...
  // 0-19: Descriptor Ring Length - "It must be 128byte-aligned (7 LS bit must
  // be set to zero)." 20-31: Reserved (0)
  for (int n = 0; n <= 127; n++) {
  ...
  // "The only time that software should write to this register is after a reset
  // (hardware reset or CTRL.RST)
  ...
  // page 596
  // Receive Descriptor Base Address Low — RDBAL[n] (0x01000 + 0x40*n, n=0...63
  // and 0x0D000 + 0x40*(n-64), n=64...127; RW)

  // 0-6: Ignored on writes, reads as 0
  // 7-31: Receive Descriptor Base Address Low
  for (int n = 0; n <= 127; n++) {
    int addr = n <= 63 ? (0x01000 + 0x40 * n) : (0x0D000 + 0x40 * (n - 64));
  ...
  // page 596
  // Receive Descriptor Base Address High — RDBAH[n] (0x01004 + 0x40*n, n=0...63
  // and 0x0D004 + 0x40*(n-64), n=64...127; RW)
  // XXX SPEC BUG page 172 point 3 says "registers RDBAL, RDBAL" but one of
  // those should be RDBAH

  // 0-31: Receive Descriptor Base Address High
  for (int n = 0; n <= 127; n++) {
    int addr = n <= 63 ? (0x01004 + 0x40 * n) : (0x0D004 + 0x40 * (n - 64));
    REG(addr, 0b00000000000000000000000000000000,
        0b11111111111111111111111111111111);
  }

  // page 596
  // Receive Descriptor Length — RDLEN[n] (0x01008 + 0x40*n, n=0...63 and
  // 0x0D008 + 0x40*(n-64), n=64...127; RW)

  // 0-19: Descriptor Ring Length - "It must be 128-byte aligned (7 LS bit must
  // be set to zero)." 20-31: Reserved (0)
  for (int n = 0; n <= 127; n++) {
    int addr = n <= 63 ? (0x01008 + 0x40 * n) : (0x0D008 + 0x40 * (n - 64));
    REG(addr, 0b00000000000000000000000000000000,
        0b00000000000011111111111110000000);
  }

  // page 597
  // Receive Descriptor Head — RDH[n] (0x01010 + 0x40*n, n=0...63 and 0x0D010 +
  // 0x40*(n-64), n=64...127; RO)

  // 0-15: Receive Descriptor Head
  // 16-31: Reserved (0)
  for (int n = 0; n <= 127; n++) {
    int addr = n <= 63 ? (0x01010 + 0x40 * n) : (0x0D010 + 0x40 * (n - 64));
    REG(addr, 0b00000000000000000000000000000000,
        0b00000000000000000000000000000000);
  }

  // page 597
  // Receive Descriptor Tail — RDT[n] (0x01018 + 0x40*n, n=0...63 and 0x0D018 +
  // 0x40*(n-64), n=64...127; RW)

  // 0-15: Receive Descriptor Tail
  // 16-31: Reserved (0)
  for (int n = 0; n <= 127; n++) {
    int addr = n <= 63 ? (0x01018 + 0x40 * n) : (0x0D018 + 0x40 * (n - 64));
    REG(addr, 0b00000000000000000000000000000000,
        0b00000000000000001111111111111111);
  }
  ...
  // page 597
  // Receive Descriptor Control — RXDCTL[n] (0x01028 + 0x40*n, n=0...63 and
  // 0x0D028 + 0x40*(n-64), n=64...127; RW)
  for (int n = 0; n <= 127; n++) {
  ...
  // XXX SPEC BUG no bit 0 mentioned, we'll assume it's reserved and reads as
  // 0... 1: Rx CRC Strip (0 - do not strip; "This bit must be set the same as
  // HLREG0.RXCRCSTRP") 2: Reserved (0) 3: DMA Init Done (1 - done) TODO change
  // this 4-16: Reserved, but reads as 0x0880 (0b1000 1000 0000) 17-21: Defines
  // a minimum packet size (after VLAN stripping, if applicable) for a packet
  // with a payload that can open a new RSC (in units of 16 byte.)
  //        (0x8 - default; "RSCFRSTSIZE is reserved for internal use. Software
  //        should set this field to 0x0")
  // 22-24: Reserved (000)
  // 25: RSC Coalescing on ACK Change (0 - default; "RSCACKC is reserved for
  // internal use. Software should set this bit to 1b") 26: FCoE Write Exchange
  ...
  // 6: Reserved (0)
  // 7-8: Local PAUSE capabilities (11 - both symmetric and asymmetric toward
  // local device) 9-11: Reserved (0) 12-13: Remote Fault (00 - no error, link
  // good) 14: Reserved (0) 15: Next Page Capable (0 - no next pages left)
  // 16-31: Reserved
  ...
  // XXX SPEC BUG the data sheet says bit 1 is reserved and set to 1, but then
  // has another bit 1... 1: RX CRC Strip (1 - enabled) 2: Jumbo Frames Enable
  // (0 - disable) 3-9: Reserved, must be set to 0x1 (!!!) 10: TX Pad Frame
  // Enable (1 - enabled) 11-14: Reserved, must be set to 0101b (!!!) 15:
  // Loopback (0 - disabled) 16: MDC Speed (1 - default) 17: Continuous MDC (0 -
  // off between packets, default) 18-19: Reserved (00) 20-23: Prepend Value (0
  // - default) 24-26: Reserved (0) 27: RX Length Error Reporting (1 - enabled,
  // default) 28: RX Padding Strip Enable (0 - disabled, default; "this
  // functionality should be used as debug mode only") 29-31: Reserved (0)
  ...
  // 2-6: Auto-negotiation Selector field (00001 - default value according to
  // 802.3ap-2007) 7-8: "Define 10 GbE PMA/PMD over four differential pairs" (01
  // - KX4 PMA/PMD, default value) 9: PMA/PMD used for 1GbE (1 - KX or BX
  // PMA/PMD, default value) 10: Disables 10GbE Parallel Detect On Dx without
  // main power (0 - no specific action) 11: Restarts auto-negotiation on
  // transition to Dx (0 - does not restart) 12: Applies new settings and
  // restarts relative auto-negotiation process (self-clearing) 13-15: Link Mode
  // Select (100 - KX/KX4/KR backplane auto-negotiation enable, Clause 37
  // negotiation disabled, default value) 16: Configures the A2 bit of the TAF
  // in the auto-negotiation word... blah blah blah... (1 - default) 17: FEC
  // Requested (0 - not requested) 18: FEC Ability; should be set to 1 only if
  // bit 16 is 1 (1 - supported) 19-22: Backplane Auto-Negotiation Rx Align
  // Treshold (0011 - default value) 23: Auto-Negotiation Rx Drift Mode (1 -
  // enabled) 24: Auto-Negotiation Rx Loose Mode (1 - enabled) 25-26:
  // Auto-Negotiation Parallel Detect Timer (00 - 1ms) 27: RF (0 - default)
  // 28-29: Pause Bits (00 - default)
  // 30-31: ...i don't even know what this description means (11 - KX supported,
  // KX4 supported)
  ...
  // 6: KX/KX4/KR Backplane Auto Negotiation Page Received (0 - not receifed;
  // clears on read) 7: Link Status (1 - link up; self-set on read to proper
  // value) 8-11: Signal Detect of 10 GbE Parallel (1111 - signal present for
  // lanes 0,1,2,3 respectively, good) 12: Signal Detet of 10 GbE serial (1 -
  // signal detected, good) 13-16: 10G Parallel lane sync status (1111 - sync
  // status OK for lanes 0,1,2,3 respectively, good) 17: 10 GbE align_status (1
  // - good) 18: 1G sync_status (1 - good) 19: KX/KX4/KR Baclplane Auto
  // Negotiation Rx Idle (0 - good) 20: PCS_1 GbE auto-negotiation enabled, aka
  // clause 37 (0 - not enabled, see AUTOC bits 13-15) 21: 1 GbE PCS enabled for
  // 1 GbE and SGMII operation (0 - not enabled) 22: 10G link enabled (1 -
  // enabled) 23: Forward Error Correction status in 10 GbE serial link (0 -
  // disabled) 24: Status of 10 GbE serial PCS (0 - disabled) 25: Status of
  // SGMII operation (0 - disabled) 26-27: MAC link mode status (11 -
  // auto-negotiation) 28-29: MAC link speec status (11 - 10 GbE) 30: Link Up (1
  // - link is up) 31: KX/KX4/KR backplane auto-negotiation completed (1 -
  // completed)
  ...
  // 0-15: Reserved (0)
  // 16-17: PMAPMD used for 10 GbE serial link operation (00 - KR; note that 01
  // and 11 are reserved) 18: Disable DME Pages Transmit (0 - not disabled)
  // 19-27: Reserved (0)
  ...
  // page 611
  // DCB Transmit Descriptor Plane Control and Status — RTTDCS (0x04900; RW)
  // DMA-Tx
  ...
  // 5: Reserved (0)
  // 6: DCB Arbiters Disable (0 - "during nominal operation this bit should be
  // set to 0") 7-16: Reserved (0) 17-19: Last Transmitted TC (0) 20-21:
  // Reserved (0) 22: Bypass Data_Pipe Monitor (1 - bypass) 23: Bypass Packet
  // Buffer Free Space Monitor (1 - bypass) 24-30: Reserved (0) 31: Link speed
  // has changed (0 - not changed)
  ...
  // 0-6: TX Descriptor Queue Index or TX Pool of Queues Index (0 - default
  // value) 7-31: Reserved (0)
  ...
  // 4-15: Reserved (0)
  // 16-31: VLAN Ether-Type a.k.a. TPID (0x8100 - "For proper operation,
  // software must not change the default setting of this field")
  REG(0x04A80, 0b10000001000000000000000000000100,
      0b00000000000000000000000000000001);

  // page 585
  // Receive Checksum Control — RXCSUM (0x05000; RW)
  // NOTE: "This register should only be initialized (written) when the receiver
  // is not enabled (for example, only write this register when RXCTRL.RXEN =
  // 0b)."

  // 0-11: Reserved (0)
  // 12: IP Payload Checksum Enable (0 - not enabled)
  // 13: RSS/Fragment Checksum Status Selection (0 - fragment checksum; 1 is
  // RSS) 14-31: Reserved (0)
  ...
  // 10: IPv6 Disable (0 - not disabled; "Internal use only – should not be set
  // to 1b") 11-13: Reserved (0) 14: IP Fragment Split Disable (0 - not
  // disabled; "Internal use only – should not be set to 1b") 15-31: Reserved
  // (0)
  ...
  // NOTE: "Before receive filters are updated/modified the RXCTRL.RXEN bit
  // should be set to 0b"
  ...
  // 0-15: VLAN Ether Type (0x8100; "For proper operation, software must not
  // change the default setting of this field") 16-27: Reserved (0) 28:
  // Canonical Form Indicator Bit Value (0 - doesn't matter since not enabled)
  ...
  // page 591
  // RSS Random Key Register — RSSRK (0x0EB80 + 4*n, n=0...9 / 0x05C80 + 4*n,
  // n=0...9; RW) NOTE: We only do the 0x05C80, DPDK doesn't use 0x0EB80
  ...
  // page 591
  // Redirection Table — RETA[n] (0x0EB00 + 4*n, n=0...31 / 0x05C00 + 4*n,
  // n=0...31; RW) NOTE: we only do the 0x05C00, DPDK doesn't use 0x0EB00
  ...
  for (int n = 0; n <= 127; n++) {
    ...
    // 7: Reserved (0)
    // 8-14: Host Threshold (0x0 - zero) TODO check that if PTHRESH is used
    // HTHRESH is >0 15: Reserved (0) 16-22: Write-Back Threshold (0x0 - zero)
    // 23-24: Reserved (0)
  ...
  // NOTE: "Programming MTQC must be done only during the init phase while
  // software must also set RTTDCS.ARBDIS
  ...
  // 0-31: VLAN Filter. Each bit i in register n ffects packets with VLAN tags
  // equal to 32n+i
  ...
  // NOTE: "The first Receive Address register [...] RAR0 should always be used
  // to store the individual Ethernet MAC address of the adapter."
  ...
  // 0-11: VLAN ID for pool filter n. (0 - no pools; note: "appears in little
  // endian order") 12-30: Reserved (0) 31: VLAN ID Enable (0 - not enabled)
  for (int n = 0; n <= 63; n++) {
  ...
  // 0-31: Pool Enable Bit Array. Bit i in 2n/2n+1 is associated with pool
  // i/32+i (0 - nothing)
  for (int n = 0; n <= 127; n++) {
  ...
  // 0-31: Word-wide bit vector in the unicast destination address filter table
  // (0 - models don't care; TODO?)
  for (int n = 0; n <= 127; n++) {
  ...
  // "This register is used by software to cause the 82599 to read individual
  // words in the EEPROM."
  ...
  //       - Software locks SWSM.SMBI by reading and getting 0 (hardware
  //       automatically sets it to 1)
  //       - Software locks SWSM.SWESMBI by writing 1 then reading 1
  //       - Software sets/clears the SW_FW_SYNC access bits it wants to by
  //       writing 1/0
  ...
  // 9: Flash firmware access (note: "Currently the FW does not access the
  // FLASH") 10-31: Reserved
  ...
  // Transmit Queue Statistic Mapping Registers — TQSM[n] (0x08600 + 4*n,
  // n=0...31; RW)
  for (int n = 0; n <= 31; n++) {
  ...
  // Queue Packets Received Drop Count — QPRDC[n] (0x01430 + 0x40*n, n=0...15;
  // RC)
  for (int n = 0; n <= 15; n++) {
  ...
  // Queue Bytes Received Count High — QBRC_H[n] (0x01038 + 0x40*n, n=0...15;
  // RC)
  for (int n = 0; n <= 15; n++) {
    REG(0x01038 + 0x40 * n, 0b00000000000000000000000000000000,
        0b00000000000000000000000000000000);
  }
  // Queue Packets Transmitted Count — QPTC[n] (0x08680 + 0x4*n, n=0...15 /
  // 0x06030 + 0x40*n, n=0...15; RC)
  for (int n = 0; n <= 15; n++) {
  ...
  // Queue Bytes Transmitted Count Low — QBTC_L[n] (0x08700 + 0x8*n, n=0...15;
  // RC)
  for (int n = 0; n <= 15; n++) {
    REG(0x08700 + 0x8 * n, 0b00000000000000000000000000000000,
        0b00000000000000000000000000000000);
  }
  // Queue Bytes Transmitted Count High — QBTC_H[n] (0x08704 + 0x8*n, n=0...15;
  // RC)
  for (int n = 0; n <= 15; n++) {
...
static struct stub_device *stub_device_get(uint64_t addr) {
...
uint64_t stub_hardware_read(uint64_t addr, unsigned offset, unsigned size) {
