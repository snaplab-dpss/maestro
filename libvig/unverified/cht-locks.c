...
int cht_fill_cht(struct VectorLocks *cht, uint32_t cht_height, uint32_t backend_capacity) {
    ...
    for (uint32_t i = 0; i < cht_height; ++i) {
        for (uint32_t j = 0; j < backend_capacity; ++j) {
            ...
            // Update the CHT
            vector_locks_borrow(cht, (int)(backend_capacity * ((uint32_t)bucket_id) + ((uint32_t)priority)), (void **)&value);
            *value = j;
            vector_locks_return(cht, (int)(backend_capacity * ((uint32_t)bucket_id) + ((uint32_t)priority)), (void *)value);
        }
    }
