...
private:
  RS3::Z3_context ctx;
  RS3::Z3_ast expression;
  std::string symbol;

public:
  NonPacketDependencyExpression(const RS3::Z3_context &_ctx,
                                const RS3::Z3_ast &_expression,
  ...
  const RS3::Z3_context &get_context() const { return ctx; }
  const RS3::Z3_ast &get_expression() const { return expression; }
...
public:
  PacketDependenciesExpression(const RS3::Z3_context &_ctx,
                               const RS3::Z3_ast &_expression,
...
protected:
  ...
  void fill_dependencies(RS3::Z3_ast &expression);
  void fill_non_packet_dependencies_expressions(RS3::Z3_ast &expression);
  ...
  Constraint(Type _type, RS3::Z3_context _ctx) : type(_type), ctx(_ctx) {}
...
public:
  ...
  get_packet_dependencies_expression(RS3::Z3_ast expression) const {
    ...
    for (const auto &packet_dependencies_expression :
      ...
      if (RS3::Z3_is_eq_ast(context, expression, current_expr)) {
  ...
  static RS3::Z3_ast parse_expr(RS3::Z3_context ctx,
...
public:
  ...
  void process(RS3::Z3_context _ctx, unsigned int _source_device,
