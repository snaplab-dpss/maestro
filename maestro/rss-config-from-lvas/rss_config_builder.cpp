...
bool RSSConfigBuilder::is_write_modifying(const std::vector<LibvigAccess> &cp,
  ...
  return !RS3::Z3_is_eq_ast(ctx, write_expr, result_expr);
}
...
RS3::Z3_ast RSSConfigBuilder::ast_equal_association(RS3::Z3_context ctx,
                                                    RS3::Z3_ast root,
                                                    RS3::Z3_ast target) {
...
RS3::Z3_ast RSSConfigBuilder::ast_replace(RS3::Z3_context ctx, RS3::Z3_ast root,
  ...
  RS3::Z3_app app = Z3_to_app(ctx, root);
  unsigned num_fields = Z3_get_app_num_args(ctx, app);
  RS3::Z3_ast *updated_args =
      (RS3::Z3_ast *)malloc(sizeof(RS3::Z3_ast) * num_fields);
...
RS3::Z3_ast RSSConfigBuilder::make_solver_constraints(
  ...
  std::stringstream ss;
  ss << "Constraints:\n";
  ss << RS3::Z3_ast_to_string(ctx, final_constraint);
  ss << "\n";
