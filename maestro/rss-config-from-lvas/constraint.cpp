...
RS3::Z3_ast Constraint::parse_expr(RS3::Z3_context ctx,
                                   const std::string &expr_str) {
  auto expr =
      RS3::Z3_parse_smtlib2_string(ctx, expr_str.c_str(), 0, 0, 0, 0, 0, 0);

  assert(RS3::Z3_get_ast_kind(ctx, expr) == RS3::Z3_APP_AST);

  RS3::Z3_app app = RS3::Z3_to_app(ctx, expr);

  RS3::Z3_func_decl app_decl = RS3::Z3_get_app_decl(ctx, app);
  RS3::Z3_symbol symbol_app_name = RS3::Z3_get_decl_name(ctx, app_decl);
  std::string app_name = RS3::Z3_get_symbol_string(ctx, symbol_app_name);

  assert(app_name == "=");
  assert(RS3::Z3_get_app_num_args(ctx, app) == 2);

  RS3::Z3_ast app_arg = RS3::Z3_get_app_arg(ctx, app, 1);
...
bool operator==(const PacketDependenciesExpression &lhs,
  ...
  return RS3::Z3_is_eq_ast(lhs.ctx, lhs.expression, rhs.expression);
}
...
std::ostream &operator<<(std::ostream &os,
  ...
  os << "  expression    ";
  os << RS3::Z3_ast_to_string(arg.ctx, arg.expression);
  os << "\n";
...
void LibvigAccessConstraint::generate_expression_from_read_args() {
  ...
  expression =
      RS3::Z3_simplify(ctx, RS3::Z3_mk_eq(ctx, first_expr, second_expr));
}
...
bool is_select_from_chunk(RS3::Z3_context &ctx, RS3::Z3_app &app,
  ...
  RS3::Z3_ast array_ast = Z3_get_app_arg(ctx, app, 0);

  // TODO: assert(Z3_get_ast_kind(ctx, array_ast) == Z3_APP_AST);

  RS3::Z3_app array_app = RS3::Z3_to_app(ctx, array_ast);
  RS3::Z3_func_decl array_decl = RS3::Z3_get_app_decl(ctx, array_app);
  RS3::Z3_symbol symbol_array_name = RS3::Z3_get_decl_name(ctx, array_decl);

  symbol_name = RS3::Z3_get_symbol_string(ctx, symbol_array_name);
...
bool is_not_chunk_symbol(RS3::Z3_context &ctx, RS3::Z3_ast &expr,
                         std::string &symbol_name) {
  RS3::Z3_ast_kind kind = RS3::Z3_get_ast_kind(ctx, expr);

  if (kind != RS3::Z3_APP_AST) {
    return false;
  }

  RS3::Z3_app app = RS3::Z3_to_app(ctx, expr);

  if (RS3::Z3_get_app_num_args(ctx, app) > 0) {
    return false;
  }

  RS3::Z3_func_decl decl = RS3::Z3_get_app_decl(ctx, app);
  RS3::Z3_symbol symbol = RS3::Z3_get_decl_name(ctx, decl);

  symbol_name = RS3::Z3_get_symbol_string(ctx, symbol);
...
void Constraint::fill_dependencies(RS3::Z3_ast &expr) {
  if (RS3::Z3_get_ast_kind(ctx, expr) != RS3::Z3_APP_AST) return;
  ...
  if (is_select_from_chunk(ctx, app, symbol_name)) {
    RS3::Z3_ast index_ast = RS3::Z3_get_app_arg(ctx, app, 1);
    // TODO: assert(Z3_get_ast_kind(ctx, index_ast) == Z3_NUMERAL_AST);

    unsigned int index;
    RS3::Z3_get_numeral_uint(ctx, index_ast, &index);
  ...
  unsigned int num_fields = RS3::Z3_get_app_num_args(ctx, app);
  for (unsigned int i = 0; i < num_fields; i++) {
    RS3::Z3_ast app_arg = RS3::Z3_get_app_arg(ctx, app, i);
...
void Constraint::zip_packet_fields_expression_and_values(
  ...
  if (first_deps.size() + second_deps.size() <
    ...
    for (const auto &pde : packet_dependencies_expressions) {
      Logger::error() << "  ";
      Logger::error() << RS3::Z3_ast_to_string(pde.get_context(),
                                               pde.get_expression());
      Logger::error() << "\n";
    }
