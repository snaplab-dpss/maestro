...
void nf_util_init() {
  ...
  // Allocating on socket 0 because NIC is pinned to node 0.
  // Whenever a new configuration is used, this should be updated accordingly.
  (*chunks_borrowed_ptr) = (void**) rte_malloc_socket(NULL, sizeof(void*) * MAX_N_CHUNKS, 0, 0);
}
